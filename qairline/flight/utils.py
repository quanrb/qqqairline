...
def addDomesticFlights():
    ...
    for i, line in tqdm(enumerate(file), total=total):
        ...
        economy_fare = int(data[11].strip()) if data[11].strip() else 0
        business_fare = int(data[12].strip()) if data[12].strip() else 0
        first_fare = int(data[13].strip()) if data[13].strip() else 0
